...
const adminController = {
    validateToken: async (req: Request, res: Response): Promise<Response> => {
        ...
        try {
            ...
            */
            ...
            logger.error(`${logPrefix} :: token :: ${token} :: error :: ${error.message}`);
            return res.status(STATUS.INTERNAL_SERVER_ERROR).send(errorCodes.auth.AUTH00000);
        }
    },
    login: async (req: Request, res: Response): Promise<Response> => {
        ...
        try {
            ...
            */
            ...
            if (error) {
                if (error.details)
                    return res.status(STATUS.BAD_REQUEST).send({ errorCode: errorCodes.auth.AUTH00000, errorMessage: error.details[0].message });
                else return res.status(STATUS.BAD_REQUEST).send({ errorCode: errorCodes.auth.AUTH00000, errorMessage: error.message });
            }
            
            logger.debug(`${logPrefix} :: Fetching user by user_name: ${user.user_name} `);
            const existingUser: IUser = await adminService.getUserByUserName(user.user_name);
            if (!existingUser) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00001);

            if (environment.decryptSensitiveData) { user.password = encDecHelper.decryptPayload(user.password); }
            if (user.password == environment.defaultPassword) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00002);
            ...
            if (isPasswordValid) {
                logger.info(`${logPrefix} :: Successful login for user: ${user.user_name}`);
                const tokenDetails = {
                ...
                const token = await generateToken.generate(existingUser.user_name, tokenDetails, environment.adminLoginExpiryTime, authConfig.AUTH.SECRET_KEY, req);
                await adminRepository.resetInvalidLoginAttempts(user.user_name);
                adminService.updateUserLoginStatus(UserStatus.LOGGED_IN, req.body.user_name, currentInvalidAttempts != 0 && currentInvalidAttempts < maximumInvalidAttempts);

                return res.status(STATUS.OK).send({
                    data: { token: token.encoded, expiryTime: `${environment.adminLoginExpiryTime}h` },
    ...
    logout: async (req: Request, res: Response): Promise<Response> => {
        ...
        try {
            ...
            */
            ...
            await redis.deleteRedis(redisKeysFormatter.getFormattedRedisKey(RedisKeys.ADMIN_USER_BY_USER_ID, { userId: user_id.toString() }));
    ...
    getForgetPasswordOtp: async (req: Request, res: Response): Promise<Response> => {
        ...
        try {
            ...
            */
            ...
            if (!userExists) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00024);

            const alreadySent = await adminService.isForgotPasswordOtpAlreadySent(mobile_number);
            if (alreadySent) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00006);
    ...
    verifyForgetPasswordOtp: async (req: Request, res: Response): Promise<Response> => {
        ...
        try {
            ...
            */
            ...
            if (environment.decryptSensitiveData) otpDetails.otp = encDecHelper.decryptPayload(otpDetails.otp);
            ...
            if (!forgotPasswordOtpDetailsCachedResult) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00007);
    ...
    resetForgetPassword: async (req: Request, res: Response): Promise<Response> => {
        ...
        try {
            ...
            */
            ...
            const { error } = await adminValidations.validateResetPassword(resetForgetPasswordDetails);
            if (error) {
                if (error.details)
                    return res.status(STATUS.BAD_REQUEST).send({ errorCode: errorCodes.auth.AUTH00000, errorMessage: error.details[0].message });
                else return res.status(STATUS.BAD_REQUEST).send({ errorCode: errorCodes.auth.AUTH00000, errorMessage: error.details[0].message });
            }

            if (environment.decryptSensitiveData) resetForgetPasswordDetails.newPassword = encDecHelper.decryptPayload(resetForgetPasswordDetails.newPassword);
            if (environment.decryptSensitiveData) resetForgetPasswordDetails.confirmPassword = encDecHelper.decryptPayload(resetForgetPasswordDetails.confirmPassword);

            if (resetForgetPasswordDetails.newPassword !== resetForgetPasswordDetails.confirmPassword) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00008);

            if (!/^(?=.*\d)(?=.*[a-zA-Z]).{8,}$/.test(resetForgetPasswordDetails.newPassword)) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00009);
            ...
            if (!forgotPasswordChangeRequestDetails) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00010);

            const parsedForgotPasswordChangeRequestDetails = JSON.parse(forgotPasswordChangeRequestDetails);

            const user: IUser = await adminService.getUserByUserName(parsedForgotPasswordChangeRequestDetails.userName);
            const isPasswordNotChanged = await bcrypt.compare(resetForgetPasswordDetails.newPassword, user.password);
            if (isPasswordNotChanged) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00019);

            const passwordHash = await bcrypt.hash(resetForgetPasswordDetails.newPassword, 10);

            const passwordResetted = await adminRepository.resetPassword(passwordHash, parsedForgotPasswordChangeRequestDetails.userName);
            if (!passwordResetted) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00011);
            redis.deleteRedis(redisKeysFormatter.getFormattedRedisKey(RedisKeys.USER_BY_USERNAME, { username: parsedForgotPasswordChangeRequestDetails.userName }));
    ...
    getLoginOtp: async (req: Request, res: Response): Promise<Response> => {
        ...
        try {
            ...
            */
            ...
            if (!mobileNumber || mobileNumber.toString().length !== 10) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00014);

            const userExists = await adminRepository.existsByMobileNumber(mobileNumber);
            if (!userExists) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00024);

            const txnId = uuidv4();
            const otp = Math.floor(100000 + Math.random() * 900000);

            const loginOtpDetails = await adminService.getLoginOTPByMobile(mobileNumber);
            if (loginOtpDetails) return res.status(STATUS.OK).send({ data: { txnId: loginOtpDetails.txnId }, message: "OTP Generated Successfully" });
            ...
            return res.status(STATUS.OK).send({ data: { txnId }, message: "OTP Generated Successfully" });
        } catch (error) {
            logger.error(`${logPrefix} :: Error :: ${error.message} :: ${error}`);
            return res.status(STATUS.INTERNAL_SERVER_ERROR).send(errorCodes.auth.AUTH00000);
        }
    ...
    verifyLoginOtp: async (req: Request, res: Response): Promise<Response> => {
        ...
        try {
            ...
            */
            const otpDetails = req.body;
            const { error } = await adminValidations.validateVerifyLoginOTP(otpDetails);
            if (error) {
                if (error.details)
                    return res.status(STATUS.BAD_REQUEST).send({ errorCode: errorCodes.auth.AUTH00000, errorMessage: error.details[0].message });
                else return res.status(STATUS.BAD_REQUEST).send({ errorCode: errorCodes.auth.AUTH00000, errorMessage: error.message });
            ...
            const loginOtpDetails = await adminService.getLoginOTPByTxnId(otpDetails.txnId);
            if (!loginOtpDetails) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00020);

            if (environment.decryptSensitiveData) otpDetails.otp = encDecHelper.decryptPayload(otpDetails.otp);
            if (loginOtpDetails.otp != parseInt(otpDetails.otp)) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00021);

            const response = await adminService.verifyLoginOTP(loginOtpDetails, req);
            if (!response.token) return res.status(STATUS.BAD_REQUEST).send(errorCodes.auth.AUTH00022);

            return res.status(STATUS.OK).send({ data: response, message: "OTP Verified Successfully" });
        } catch (error) {
            logger.error(`${logPrefix} :: Error :: ${error.message} :: ${error}`);
            return res.status(STATUS.INTERNAL_SERVER_ERROR).send(errorCodes.auth.AUTH00000);
        }
